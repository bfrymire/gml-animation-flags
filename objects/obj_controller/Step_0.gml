...
if _dir != 0 {
    var _flag_position = animator.get_flag_position(animator.active_flag);
    _flag_position += _dir;
    if _flag_position < 0 {
        _flag_position = animator.get_flags_number() - 1;
    } else if _flag_position > animator.get_flags_number() - 1 {
        _flag_position = 0;
    }
    flag_picker_box.offset_want = _flag_position * string_height("M");
    animator.set_flag(animator.get_flag_at_position(_flag_position).name);
}
